...
<body>
    ...
    <div class="mt-10 mb-10 mx-auto max-w-sm pl-6 flex">
        <div class=" mt-2"><a href="/clientnotifications" class="bg-gray-500 hover:bg-gray-700 text-white py-2.5 px-4 mt-2 rounded">Go Back</a></div>
        <form action="/clientnotifications/{{ $clientnotification->id }}" method="POST">
            @csrf
            @method('DELETE')
    ...
    $('.deletebtn').click(function (e){
      ...
      .then((willDelete) => {
        if (willDelete) {
          ...
          $.ajax({
            type: "DELETE",
            url: '/clientnotifications/'+ delete_id,
            data: data,
